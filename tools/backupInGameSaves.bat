...
REM : main
    ...
    for /F "delims=~" %%i in ('dir /b /o:n !pat! 2^>NUL') do (
        ...
        !rarExe! a -ed -ap"mlc01\emulatorSave" -ep1 -r -inul -w"!BFW_PATH:"=!logs"  !rarFileEmuSave! !folder! > NUL 2>&1
    )
...
REM : functions
    ...
    :compress
        ...
        if exist !sf! (
            !rarExe! a -ed -ap"mlc01\usr\save\%~1" -ep1 -r -inul -w"!BFW_PATH:"=!logs" !rarFile! !sf! > NUL 2>&1
            set "%1=!ERRORLEVEL!"
        )
